...
class GamePanel extends StatefulWidget {
  ...
  @override
  State<GamePanel> createState() => GamePanelState();
...
class GamePanelState extends State<GamePanel> {
  ...
  _fillOneEmptyCell() {
    ...
    if (emptyCellPosition != -1) {
      _cellArr[(emptyCellPosition / 4).toInt()][emptyCellPosition % 4] =
          _getRandomValue();
    }
  }
  ...
  _panelOnUpdate(DragUpdateDetails details, Offset lastPosition) {
    ...
    } else if ((currentPosition.dx - lastPosition.dx).abs() >
            _mainAxisMinLimit &&
    ...
    if (toHead) {
      ...
      for (var i = 0; i < newArr.length - 1; i++) {
        if (newArr[3 - i] == newArr[2 - i] && newArr[3 - i] != 0) {
          newArr[3 - i] = 0;
          newArr[2 - i] *= 2;
          _currentScore += (newArr[2-i] as int);
          widget.onScoreChanged?.call(_currentScore);
        }
      }
    } else {
      for (var i = 0; i < arrWithoutZero.length; i++) {
        newArr[newArr.length - i - 1] =
            arrWithoutZero.elementAt(arrWithoutZero.length - i - 1);
      }
      ...
      for (var i = 0; i < newArr.length - 1; i++) {
        if (newArr[i] == newArr[i + 1] && newArr[i] != 0) {
          newArr[i] = 0;
          newArr[i + 1] *= 2;
          _currentScore += (newArr[i+1] as int);
          widget.onScoreChanged?.call(_currentScore);
        }
      }
    }
    ...
    return Container(
      ...
      child: Center(
        child: Text(
          ...
          style: TextStyle(
            color: value == 2 || value == 4
                ? ColorUtil.textColor1
                : ColorUtil.textColor3,
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
                style: ButtonStyle(
                  backgroundColor:
                      MaterialStateProperty.all<Color>(ColorUtil.bgColor3),
                ),
                onPressed: () {
                  startGame();
                  setState((){
